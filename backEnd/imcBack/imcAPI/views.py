...
@csrf_exempt
def upload_facturas_file(request):
    log_file = Path(settings.BASE_DIR) / "facturas_upload_debug.log"

    with open(log_file, "a") as log:
        log.write("\n===== Facturas File Upload Attempt =====\n")
        log.write("Received request for file upload\n")
        ...
        if request.method == 'POST' and request.FILES.get('file'):
            ...
            # Set script path
            script_path = Path("/root/IMC/Proyecto IMC/Scripts/data_cleaner/facturas_cleaning_script.py").resolve()
            log.write(f"Script path is set to: {script_path}\n")
...
@api_view(['GET'])
def get_last_10_facturas(request):
    client = bigquery.Client()

    query = """
    SELECT Date, Num, Partner, Pais, Territory, Memo_Description, Amount, INVOICE_TYPE
    FROM `imc-storage.imcData.valores_facturacion`
    ORDER BY Date DESC
    LIMIT 10
    """
    query_job = client.query(query)
